<template>
  <div>
    <div class="block-interval">
      <a-card :bordered="false">
        <a-form layout="inline" :model="where">
          <a-row>
            <a-form-item label="销货商账号:">
              <a-input
                v-model:value.trim="where.account"
                placeholder="请输入销货商账号"
                allow-clear
              />
            </a-form-item>
        ...
        >
          ...
          <template #bodyCell="{ text, column, record }">
            ...
            <template v-if="column.dataIndex === 'settlement'">
              <a @click="settlementOperation(record)">结算</a>
            </template>
    ...
    <!-- 结算 -->
    <a-modal
      v-model:visible="settlementOperationVisibleOperation"
      title="余额增减"
    ...
    >
      <div>
        <a-form
          ...
          :model="settlementOperationObj"
          ref="settlementOperationFormRef"
          name="custom-validation"
          :rules="rules1"
        >
          <a-form-item label="可用余额" name="type" :labelCol="{ span: 4 }">
            <span style="color: red">
              {{ Number(settlementOperationObj.balance).toFixed(2) }}&nbsp;</span
            >元
          </a-form-item>
          <a-form-item has-feedback label="操作" name="type" :labelCol="{ span: 4 }">
            <a-radio-group v-model:value="settlementOperationObj.type" name="type">
          ...
          <a-form-item has-feedback label="金额" name="amount" :labelCol="{ span: 4 }">
            <a-input
              type="number"
              style="width: 100%"
              v-model:value="settlementOperationObj.amount"
              placeholder="金额"
              min="0"
            />
          </a-form-item>
          <a-form-item has-feedback label="备注" name="remark" :labelCol="{ span: 4 }">
            <a-input
              style="width: 100%"
              v-model:value="settlementOperationObj.remark"
              placeholder="备注"
            />
          </a-form-item>
        </a-form>
...
<script lang="ts" setup>
  ...
  const columns = ref<object[]>([
    ...
    { title: '待结算金额', dataIndex: 'unsettledBalance' },
    { title: '已结算金额', dataIndex: 'settlableBalance' },
    { title: '结算', dataIndex: 'settlement', width: 100 },
  ...
  // 结算
  const settlementOperationVisibleOperation = ref(false);
  const settlementOperationObj = ref({
    balance: '',
    amount: '',
    merchantType: 2,
    remark: '',
  ...
  const settlementOperationFormRef = ref();
  function settlementOperation(record) {
    settlementOperationObj.value.balance = record.balance;
    settlementOperationObj.value.userId = record.employeeId;
    settlementOperationVisibleOperation.value = true;
    nextTick(() => {
      settlementOperationFormRef.value.resetFields();
    });
  }
  function confirmCreditOperationLog(record) {
    settlementOperationFormRef.value
      .validate()
      .then(() => {
        confirmSettlementOperations();
      })
      .catch((error) => {
        console.log('error', error);
      });
  }
  async function confirmSettlementOperations() {
    try {
      let queryResult = await merchantApi.awardCredit(settlementOperationObj.value);
      if (queryResult.code === 0) {
        query();
        message.success('操作成功');
        settlementOperationVisibleOperation.value = false;
      }
    } catch (e) {
  ...
  async function confirmDeleteMerchant(record) {
    try {
      // await merchantApi.merchantStatus({ userId: record.userId, type: 2 });
      message.success('操作成功');
      reload();
    } catch (e) {
      message.error('修改失败');
